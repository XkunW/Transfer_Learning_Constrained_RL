...
class TabularSF:
    ...
    def update_cost(self, phi, c, task_index, exact=False):
        """
        Updates the cost parameters for the given task based on the observed cost sample
        ...
        phi : np.ndarray
            the state features
        c : float
            the observed cost from the MDP
        task_index : integer
            the index of the task from which this cost was sampled
        exact : boolean
            if True, validates the true cost from the environment and the linear representation
        """

        # update cost using linear regression
        w = self.fit_w[task_index]
        phi = phi.reshape(w.shape) # Update this
        c_fit = np.sum(phi * w) # Update this
        self.fit_w[task_index] = w + self.alpha_w * (c - c_fit) * phi # Update this

        # validate cost
        c_true = np.sum(phi * self.true_w[task_index]) # Update this
        if exact and not np.allclose(c, c_true):
            raise Exception(
                "sampled cost {} != linear cost {} - please check task {}!".format(
                    c, c_true, task_index
                )
            )
