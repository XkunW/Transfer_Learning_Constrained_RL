...
class TrajectoryConstrainedQLV2(Agent):
    def __init__(
        self, 
        learning_rate, 
        threshold,
    ...
    def train_agent(self, s, s_enc, a, r, c, s1, s1_enc, gamma):
        ...
        for action in range(self.n_actions):
            if j1[action] + self.cost_since_last_episode > self.task_threshold:
                # temporarily set unsafe action q value to negative inf
                safe_actions.remove(action)
                q1[action] = -float('inf')

        # if no safe actions for next state under current estimation, ignore safety when doing update and start new episode
        if not safe_actions:
        ...
        self.J[s[0]][a] += self.alpha * (target_j - self.J[s[0]][a])
    ...
    def _epsilon_greedy(self, q):
        ...
        # if no safe actions from current estimation, start new episode
        if not safe_actions:
